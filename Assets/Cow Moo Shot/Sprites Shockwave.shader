...
Shader "TSH/Sprites Shockwave"
{
    Properties
    {
        ...
        _MinRadians ("Min Degree Radians", Range (-3.15, 3.14)) = -3.15 //outer radius, in local coordinates
		 _MaxRadians ("Max Degree Radians", Range (-3.15, 3.14)) = 3.14 //outer radius, in local coordinates
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : COLOR
            {
				...
				half2 testUV = normalize(i.uv - i.uv_center);
				float radians = atan2(0 - testUV.y, 0 - testUV.x);

                if(dist > 0 && dist < 1 && radians > _MinRadians && radians < _MaxRadians)
                {
